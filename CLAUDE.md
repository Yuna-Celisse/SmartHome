...
- **`voice_protocol`**：5 字节协议解析（`AA 55 [type] [cmd] FB`），`Voice_Process_Byte()` 逐字节组包，dispatch 风扇 0x01–0x05 和灯光 0x0A/0x0B 命令。
- **`main.c`**：
  - 初始化顺序：`SYSCFG_DL_init()` → `Board_Sensor_Enable()` → `Board_I2C_Init()` → `Board_Sensor_Init()` → UART0/1/3 Init → `FanControl_Init()` → `Board_Buzzer_Init()` → `OPT3001_Init()` → `ESP_Init()` → `IoTDA_Init()`
